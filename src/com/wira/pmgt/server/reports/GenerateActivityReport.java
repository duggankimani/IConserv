...
public class GenerateActivityReport {
	 ...
	 private static final String[] titles = {
		 "Overall LWF Goals","Activity","Targets","Indicator","Funding", "Status","Remarks","Monitoring test"};
	...
	public static void main(String[] args) throws Exception {
        ...
        List<ProgramDetail> lst = DB.getProgramDaoImpl().getProgramDetails(ProgramDetailType.PROGRAM, DB.getProgramDaoImpl().getActivePeriod());
        for(ProgramDetail d: lst){	
	        IsProgramDetail singleResult = ProgramDaoHelper.getProgramById(d.getId(), true);
	        ...
	        List<IsProgramDetail> data = singleResult.getProgramOutcomes();
	        if(data!=null){
	        	generate(wb, singleResult.getName(), data);
	        }
        }
	...
	public static void generate(Workbook wb,String programName,	List<IsProgramDetail> data){
	...
	private static void bindData(IsProgramDetail detail,CreationHelper helper,Sheet sheet, Row row, int i) {
		...
		for (int j = 0; j < titles.length; j++) {
            ...
            switch(j){
                ...
                case 2:
                    ...
                    if(!isHeader){
                    	generateAndFormatValue(helper,cell, detail.getTargetsAndOutcomes());
                    }
                ...
                case 3:
                    ...
                    styleName = isHeader ? "cell_b" : "cell_normal";
                    if(!isHeader){
                    	generateAndFormatValue(helper,cell, detail.getTargetsAndOutcomes(), true);
                    }
	...
	private static void generateAndFormatValue(CreationHelper helper,Cell cell,
			List<TargetAndOutcomeDTO> targetsAndOutcomes){
		generateAndFormatValue(helper, cell, targetsAndOutcomes, false);
	}
	
	private static void generateAndFormatValue(CreationHelper helper,Cell cell,
			List<TargetAndOutcomeDTO> targetsAndOutcomes, boolean indicatorsOnly) {
		int count=0;
    	...
    	RichTextString richText = helper.createRichTextString(out.substring(0, out.length()-2));
    	...
    	for(i=0; i<sizes.length; i++){
    		...
    		if(indicatorsOnly){
    			...
    			TargetAndOutcomeDTO dto= targetsAndOutcomes.get(i);
       		 	font_style= dto.getActualOutcome()==null? "_nodata":
       		 		dto.getActualOutcome()> dto.getTarget() ? "font-success": "font-failure";
    		}
    			
    		richText.applyFont(count+=2,count=count+sizes[i],fonts.get(font_style));
    	}
